  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
        ...
        <span className="text-sm text-slate-500">{passed}/{total} passed</span>
      </div>
...
function ReportContent() {
  ...
  useEffect(() => {
    async function loadOrFetch() {
      ...
      // Try sessionStorage first (coming from form submission)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center gradient-bg">
        <div className="text-center text-white">
          ...
          <h2 className="text-xl font-semibold mb-2">Auditing your website...</h2>
          <p className="text-slate-300">This usually takes 5–15 seconds</p>
        </div>
      </div>
    )
  }
  ...
  const scoreLabel = result.score >= 80 ? 'Great' : result.score >= 50 ? 'Needs Work' : 'Poor'
  const shareUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/report?url=${encodeURIComponent(urlParam || '')}`
    : ''

  const whatsappMessage = encodeURIComponent(
    `Hi! I just ran a website audit on ${result.url} and scored ${result.score}/100. I'd like help improving my website. Can we discuss?`
  )
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-sm text-slate-400">
                Score: <strong className="text-white">{result.score}/100</strong> — {scoreLabel}
                {' '} · Completed in {(result.auditTime / 1000).toFixed(1)}s
              </p>
          ...
          <div className="gradient-bg p-8 md:p-12 text-center text-white">
            <h2 className="text-2xl md:text-3xl font-bold mb-3">
              {result.score < 70
                ...
                : 'Want to Score Even Higher?'}
            </h2>
            <p className="text-slate-300 mb-8 max-w-xl mx-auto">
              {result.score < 70
                ? 'Our team can fix these issues and optimize your website to convert more visitors into paying customers.'
                : 'Our experts can fine-tune your site for maximum performance and lead generation.'}
            </p>
            ...
            <p className="mt-6 text-sm text-slate-400">
              Free consultation — no obligation. Let us help you grow.
            </p>
          </div>
        </div>
